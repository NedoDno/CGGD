...
namespace cg::renderer
{
	...
	{
		float frame_weidht = 1.f/static_cast<float>(accumulation_num);
...
#pragma omp parallel for
			for(int x=0; x<width;x++){
				for(int y=0;y<height;y++){
					...
					history_pixel += sqrt(float3{payload.color.r, payload.color.g, payload.color.b} * frame_weidht);
	...
	{
		float3 inc_ray_direction = float3(1.f)/ray.direction;
		float3 t0 = (aabb_max- ray.position)*inc_ray_direction;
		float3 t1 = (aabb_min- ray.position)*inc_ray_direction;
